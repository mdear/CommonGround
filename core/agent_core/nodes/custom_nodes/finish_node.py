...
class FinishNode(AsyncNode):
    async def prep_async(self, shared: Dict) -> Dict:
        ...
        # Extract deliverables from message history
        extracted_deliverables = {}
        
        if is_principal:
            # For Principal: Extract the final report from the last assistant message
            # This is the markdown report generated by generate_markdown_report
            messages = shared.get("state", {}).get("messages", [])
            final_report_content = None
            
            # Look for the last substantial markdown content (final reports start with #)
            for msg in reversed(messages):
                if msg.get("role") == "assistant":
                    ...
                    if content and content.strip().startswith("#") and len(content) > 5000:
                        ...
                        logger.info("finish_flow_principal_report_extracted", extra={
                            "char_count": len(content),
                            "title_preview": content.split('\n')[0][:80]
                        })
                        break
            
            if final_report_content:
                # Store as final_report for the Partner to access
                extracted_deliverables = {
                    "final_report": final_report_content,
                    "final_report_char_count": len(final_report_content)
                }
            
            # Also preserve any existing primary_summary (execution plan)
            existing_deliverables = shared.get("state", {}).get("deliverables", {})
            if existing_deliverables.get("primary_summary"):
                extracted_deliverables["primary_summary"] = existing_deliverables["primary_summary"]
        
        elif parent_agent_id_from_meta:
